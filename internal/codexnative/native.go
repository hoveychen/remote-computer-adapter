...
func nativeService(state *trustedstate.Store, token string) (http.Handler, func(string) error, string, error) {
	initial, err := trustedstate.NativeHTTPHandler(state, []trustedstate.NativeCredential{{Token: token, Kind: "model_tool", ThreadID: "bootstrap"}})
	if err != nil {
		return nil, nil, "", err
	}
	...
	bind := func(threadID string) error {
		...
		next, err := trustedstate.NativeHTTPHandler(state, []trustedstate.NativeCredential{{Token: token, Kind: "model_tool", ThreadID: threadID}})
		if err != nil {
			return err
		}
